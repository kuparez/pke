...
import pymorphy2
from keyverbum.keywords import PymorphyStemmer
...
class LoadFile(object):
    ...
    def load_document(self, input, **kwargs):
        ...
        if language not in ISO_to_language:
            logging.warning("ISO 639 code {} is not supported, switching to 'en'.".format(language))
            language = "en"
    ...
    def apply_stemming(self):
        ...
        elif self.language == "ru2":
            morph = pymorphy2.MorphAnalyzer()
            stemmer = PymorphyStemmer(morph)
            stemmer.stem = stemmer.transform
        else:
            # create a new instance of a porter stemmer
            stemmer = SnowballStemmer(ISO_to_language[self.language], ignore_stopwords=True)
    ...
    def candidate_filtering(
        ...
        for k in list(self.candidates):
            ...
            if only_alphanum and k in self.candidates:
                if not all([self._is_alphanum(w, valid_punctuation_marks) for w in words]):
                    del self.candidates[k]
