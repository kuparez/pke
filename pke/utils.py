...
def train_supervised_model(
    ...
    for input_file in glob.iglob(input_dir + "/*." + extension):
        ...
        # load the document
        model.load_document(input=input_file, language=language, normalization=normalization)
    ...
    with codecs.open(input_file, "r", encoding) as f:
        ...
        if input_file.endswith(".json"):
            ...
            for doc_id in references:
                references[doc_id] = [
                    keyphrase for variants in references[doc_id] for keyphrase in variants
                ]
        ...
        if normalize_reference:
            ...
            if language != "en":
                stemmer = SnowballStemmer(ISO_to_language[language], ignore_stopwords=True)
...
def compute_lda_model(
    input_dir, output_file, n_topics=500, extension="xml", language="en", normalization="stemming"
):
    ...
    # stopwords atm
    tf_vectorizer = CountVectorizer(stop_words=stopwords.words(ISO_to_language[language]))
...
def load_document_as_bos(input_file, language="en", normalization="stemming", stoplist=None):
...
def compute_pairwise_similarity_matrix(
    ...
    for input_file in glob.iglob(input_dir + "/*." + extension):
        ...
        documents[input_file] = load_document_as_bos(
            input_file=input_file, language=language, normalization=normalization, stoplist=stoplist
        )
